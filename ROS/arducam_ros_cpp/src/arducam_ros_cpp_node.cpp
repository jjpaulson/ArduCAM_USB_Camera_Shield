...
class trigger {
	public:
		...
		unsigned int trigger_count_val;
		bool trigger_val;
		void trigger_time_callback(const std_msgs::TimeConstPtr& trigger_time);
		void trigger_count_callback(const std_msgs::UInt32ConstPtr& trigger_count);
		void trigger_callback(const std_msgs::BoolConstPtr& trigger);
};

void trigger::trigger_time_callback(const std_msgs::TimeConstPtr& trigger_time) {
	this->trigger_time_val = trigger_time->data;
}

void trigger::trigger_count_callback(const std_msgs::UInt32ConstPtr& trigger_count) {
	this->trigger_count_val = trigger_count->data;
}

void trigger::trigger_callback(const std_msgs::BoolConstPtr& trigger) {
	this->trigger_val = trigger->data;
}
...
void captureImage_thread(ArduCamHandle handle, ros::NodeHandle * nh) {
	...
	while (_running) {
		...
		if ( rtn_val == USB_CAMERA_USB_TASK_ERROR) {
			...
			Uint32 * pval;
			rtn_val = ArduCam_readSensorReg(handle, 12336, pval);
...
int main(int argc,char **argv)
{
	...
	if (camera_initFromFile(config_file_name, cameraHandle, cameraCfg)) {
		...
		std::thread captureThread(captureImage_thread, cameraHandle, &nh);
		std::thread readThread(readImage_thread, cameraHandle, &nh);
